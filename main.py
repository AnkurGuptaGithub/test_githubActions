...
df.GMP = df.GMP.str.replace(',','').str.replace('₹','').str.replace('--','0').str.replace('NA','0').str.split('(', expand=True)[0].astype('float')
df.Price = df.Price.str.replace(',','').str.replace('--','0').str.replace('NA','0').astype('float')
df.Name = df.Name.apply(lambda x: ' '.join(x.split(' ')[:-1])).str.replace(' &', '')
